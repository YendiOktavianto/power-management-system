...
export default function PowerFactorGauge({ powerFactor, needleAngle }: {
  powerFactor: number;
  needleAngle: number;
}) {
  return (
    <div className="flex flex-col items-center w-full md:w-1/2 bg-[#032d7a] rounded-2xl p-6">
      <svg viewBox="0 0 200 200" className="w-56 h-56">
        {/* arc segments sama seperti kode asli */}
        ...
        {Array.from({ length: 6 }).map((_, i) => {
          const val = (i * Y_DOMAIN[1]) / 5;
          const angle = START_ANGLE + (i / 5) * (END_ANGLE - START_ANGLE);
