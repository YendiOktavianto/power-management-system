...
import { API_ADMIN_REPORT_ENERGY } from "./constants";
...
export default function useEnergyUsageReport() {
  ...
  // fetch data dari BE
  useEffect(() => {
    // ⛔ Jangan fetch kalau belum ada tanggal ATAU belum ada lokasi
    ...
    params.set("to", to);
    // selectedLocation sudah pasti ada
    params.set("deviceId", selectedLocation);

    const url = `${API_ADMIN_REPORT_ENERGY}?${params.toString()}`;
    console.log("[ENERGY ADMIN] fetch:", url);
    ...
    const load = async () => {
      ...
      if (!res.ok) {
        ...
        console.error("[useEnergyUsageReport] error:", err);
        setData([]);
        setTotalRows(0);
      }
    };
